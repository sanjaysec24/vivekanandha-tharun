  ...
  return (
    <nav id="navbar-container" className="sticky top-0 z-50 bg-[#F4F0EA]/95 backdrop-blur-md border-b border-[#3A2318]/10 px-6 py-4 md:px-12">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        {/* Brand Logo */}
        <a id="brand-logo" href="#" className="flex items-center space-x-2 text-2xl font-serif font-bold text-[#3A2318] tracking-tight group">
        ...
        <div id="desktop-menu" className="hidden md:flex items-center space-x-8">
          {navigationItems.map((item) => (
            ...
            >
              <a
                href={item.href}
                className="flex items-center text-sm font-medium text-[#3A2318] hover:text-[#E78F68] transition-colors duration-200 py-2"
              >
                {item.label}
                {item.hasDropdown && (
        ...
        <div className="hidden md:flex items-center space-x-4">
          <button
            ...
            className="flex items-center space-x-1 border border-[#3A2318]/30 text-sm font-semibold text-[#3A2318] px-5 py-2.5 rounded-full hover:bg-[#3A2318] hover:text-[#F4F0EA] transition-all duration-200 shadow-sm"
          >
              ...
              {navigationItems.map((item) => (
                <div key={item.label} className="space-y-2">
                  <a
                    ...
                    className="block text-base font-semibold text-[#3A2318] hover:text-[#E78F68] px-2 py-1"
                  >
                    {item.label}
                  </a>
                  {item.hasDropdown && (
